...
if o=='1':
    ...
    correo(source,date,hour,lat,lon,dep,M.split(' ')[1],region,GFZ_ID,map, link_gfz)
    sfile_name=sfile_builder(date,hour,lat,lon,dep,M.split(' ')[1],source)
    print sfile_name
elif o=='2':
    print '=======USGS======='
    map = EQ_map(lon2,lat2,M.split(' ')[1],source2,region2, USGS_ID)
    correo(source2,date2,hour2,lat2,lon2,dep2,M2.split(' ')[1],region2,USGS_ID,map, link_usgs)
    sfile_name=sfile_builder(date2,hour2,lat2,lon2,dep2,M2.split(' ')[1],source2)
    print sfile_name
else:
	sys.exit()	    
